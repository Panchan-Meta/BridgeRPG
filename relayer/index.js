...
// PGirlsChain ̊ǗEHbgiubWMint/Burnpj
...
//  amount  PGirls ̍ŏP(10^decimals; ʏ1e18)œnĂO
...
app.post("/bridge-eth", async (req, res) => {
  ...
  try {
    ...
    // ؁ichainId ̓T[oݒlɌŒ肵Abody chainId Ƃ͕ʂɌ؁j
    ...
    await tx.wait(1); // 1 confirmation ȏ㐄

    usedNonces.add(bnNonce.toString());

    // NOTE: tH[}bg͎wʂŒBPGirlsETH ł ETH TxHash ͑gUNVLڂ܂B
    const content = completedMsg({
      user,
      from: "PGirls",
      to: "ETH",
      amount: fmt(bnAmount, 18), // PGirls amountiFrom ̐ʁj
